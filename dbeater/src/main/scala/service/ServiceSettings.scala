...
class ServiceSettings(system: ExtendedActorSystem) extends Extension {
  ...
  val activeMQAddress: String = system.settings.config getString "activeMQ.address"
  val activeMQPort: Int = system.settings.config getInt "activeMQ.port"

  val elastic: String = system.settings.config getString "elastic.address"
}